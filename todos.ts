...
function manualTodosHelp(): string {
	return [
		"Usage:",
		"  /todos                         Open todo manager",
		"  /todos list                    List open/assigned todos",
		"  /todos list-all                List all todos",
		"  /todos get <id>",
		"  /todos create <title> [--body <text>] [--tags a,b] [--status open]",
		"  /todos update <id> [--title <text>] [--body <text>] [--tags a,b] [--status open]",
		"  /todos append <id> <text>",
		"  /todos claim <id> [--force]",
		"  /todos release <id> [--force]",
		"  /todos delete <id>",
		"  /todos close <id>",
		"  /todos reopen <id>",
	].join("\n");
...
function reportManualTodoResult(ctx: ExtensionContext, text: string, kind: "info" | "error" = "info"): void {
	console.log(text);
	if (ctx.hasUI) {
		const firstLine = text.split("\n")[0] || text;
		ctx.ui.notify(firstLine, kind);
	}
}

async function runManualTodosCommand(argsText: string, todosDir: string, ctx: ExtensionContext): Promise<boolean> {
	const argv = splitCommandArgs(argsText);
	const command = argv.shift()?.toLowerCase();
	if (!command) return false;

	if (command === "help" || command === "--help" || command === "-h") {
		reportManualTodoResult(ctx, manualTodosHelp());
		return true;
	}
	...
	if (command === "list" || command === "ls") {
		...
		reportManualTodoResult(ctx, formatTodoList([...assignedTodos, ...openTodos]));
		return true;
	}

	if (command === "list-all" || command === "all") {
		reportManualTodoResult(ctx, formatTodoList(await listTodos(todosDir)));
		return true;
	}
	...
	if (command === "get" || command === "show") {
		const id = argv.shift();
		if (!id) {
			reportManualTodoResult(ctx, "Error: id required", "error");
			return true;
		}
		const validated = validateTodoId(id);
		if ("error" in validated) {
			reportManualTodoResult(ctx, validated.error, "error");
			return true;
		}
		const todo = await ensureTodoExists(getTodoPath(todosDir, validated.id), validated.id);
		reportManualTodoResult(ctx, todo ? serializeTodoForAgent(todo) : `Todo ${displayTodoId(id)} not found`, todo ? "info" : "error");
		return true;
	}
	...
	if (command === "create" || command === "new" || command === "add") {
		...
		const status = takeFlagValue(argv, "--status") ?? "open";
		const title = argv.join(" ").trim();
		if (!title) {
			reportManualTodoResult(ctx, "Error: title required", "error");
			return true;
		}
		...
		if (typeof result === "object" && "error" in result) {
			reportManualTodoResult(ctx, result.error, "error");
			return true;
		}
		reportManualTodoResult(ctx, `Created ${formatTodoId(id)} ${title}`);
		return true;
	}
	...
	if (command === "update" || command === "edit") {
		...
		if (!existsSync(filePath)) {
			reportManualTodoResult(ctx, `Todo ${displayTodoId(id)} not found`, "error");
			return true;
		}
		...
		reportManualTodoResult(ctx, `Updated ${formatTodoId(validated.id)}`);
		return true;
	}
	...
	if (command === "append") {
		...
		if (!id || !text) {
			reportManualTodoResult(ctx, "Error: id and text required", "error");
			return true;
		}
		const validated = validateTodoId(id);
		if ("error" in validated) {
		...
		reportManualTodoResult(ctx, `Appended to ${formatTodoId(validated.id)}`);
		return true;
	}

	if (["claim", "release", "delete", "close", "reopen"].includes(command)) {
		...
		let result: TodoRecord | { error: string };
		if (command === "claim") result = await claimTodoAssignment(todosDir, id, ctx, force);
		else if (command === "release") result = await releaseTodoAssignment(todosDir, id, ctx, force);
		else if (command === "delete") result = await deleteTodo(todosDir, id, ctx);
		else result = await updateTodoStatus(todosDir, id, command === "close" ? "closed" : "open", ctx);
		if ("error" in result) {
			reportManualTodoResult(ctx, result.error, "error");
			return true;
		}
		...
		reportManualTodoResult(ctx, `${labels[command] ?? command} ${formatTodoId(result.id)}`);
		return true;
	}
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("todos", {
		description: "Manage todos from .pi/todos",
		handler: async (args, ctx) => {
			const todosDir = getTodosDir(ctx.cwd);
			const searchTerm = (args ?? "").trim();
			if (searchTerm && await runManualTodosCommand(searchTerm, todosDir, ctx)) {
				return;
			}
			if (searchTerm && !ctx.hasUI) {
				reportManualTodoResult(ctx, manualTodosHelp(), "error");
				return;
			}
